...
class SDSPIComponent : public Component, public spi::SPIDevice {
 public:
  ...
  bool write_csv_row(const char *path, const std::vector<std::string> &columns, bool append = true);
  bool read_csv_row(const char *path, std::vector<std::string> &columns, size_t row_index);
  size_t count_csv_rows(const char *path);
  bool append_csv_header(const char *path, const std::vector<std::string> &headers);
