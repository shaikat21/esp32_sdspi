...
TYPES = [CONF_USED_SPACE, CONF_TOTAL_SPACE, CONF_FREE_SPACE, CONF_FILE_SIZE]
...
async def to_code(config):
    sd_spi_component = await cg.get_variable(config[CONF_SD_SPI_CARD_ID])
    var = await sensor.new_sensor(config)
    if config[CONF_TYPE] in SIMPLE_TYPES:
        func = getattr(sd_spi_component, f"set_{config[CONF_TYPE]}_sensor")
        cg.add(func(var))
    elif config[CONF_TYPE] == CONF_FILE_SIZE:
        cg.add(sd_spi_component.add_file_size_sensor(var, config[CONF_PATH]))
